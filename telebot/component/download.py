...
def download_from_url(update: Update, context: CallbackContext):
    ...
    res = qclient.add_link(link=torrent_url,
                           save_path=save_path,
                           category=category,
                           is_paused=False)
...
def download_from_file(update: Update, context: CallbackContext):
    ...
    qclient.add_torrent(file_name=file_path,
                        save_path=save_path,
                        category=category,
                        is_paused=False)
...
updater.add_handler(MessageHandler(Filters.text &
                    Filters.regex(r'^magnet:\?.*'), download_from_magnet))
updater.add_handler(MessageHandler(Filters.text &
                    Filters.regex(r"^https?:\/\/.*(jackett|\.torren|\/torrent).*"), download_from_url))
